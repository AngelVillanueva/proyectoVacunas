...
<h2>Vacunas con fecha de vacunación <?php echo $fechavac; ?></h2>
			...
			foreach ($data as $vaccination) {
				...
				echo $this->Html->tag('td', $vaccination['Vaccination']['id'], array('class'=>'hidden'));
					...
					foreach ($matrizHeadings as $heading=>$field) {
						...
						echo $this->Html->tag('td', $this->Text->truncate($celda, 20, array('ending'=>'...', 'exact'=>true)), array('title'=>$titlecelda, 'class'=>$field));
						$contador ++;
					}
				if($role == 1)
				{
					...
					'</td><td class="tableaction editpac">'.
					$this->Html->link('Paciente', array('controller' => 'patients', 'action' => 'edit/'.$vaccination['Patient']['id'])).
					'</td>';
				}
				echo '</tr>';
			}
